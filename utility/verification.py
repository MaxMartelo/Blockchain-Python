...
class Verification:
    ...
    def verify_transaction(transaction, get_balance, check_funds = True):
        ...
        Args:
            ...
            check_funds: in the case of a node, there is no need for this check 

        """
        if check_funds:
            sender_balance = get_balance()
            return sender_balance >= transaction.amount and Wallet.verify_transaction(transaction) # = Does the sender have enough money to do the transaction ? and is the signature of the transaction valid ? 
        else:
